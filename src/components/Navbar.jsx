  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'py-3 ' + theme.glass + ' border-b ' + theme.border : 'py-5 bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        {/* Links de navegación */}
        <div className={`hidden md:flex items-center gap-8 lg:gap-10 text-[12px] font-bold tracking-[0.15em] ${isDark ? 'text-white' : 'text-[#0f172a]'}`}>
          {Object.entries(t.nav).filter(([key]) => key !== 'login').map(([key, label]) => (
            <a 
              key={key} 
              href={`#${key}`} 
              className={`opacity-70 hover:opacity-100 transition-all hover:-translate-y-0.5 relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:transition-all hover:after:w-full ${isDark ? 'after:bg-white' : 'after:bg-[#0f172a]'}`}
            >
              {label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3 lg:gap-4">
          <button 
            ...
            className={`text-[11px] font-black px-3 py-2.5 rounded-lg border transition-all hover:scale-105 ${
              isDark 
                ? 'border-white/20 hover:border-white/40 hover:bg-white/5 text-white' 
                : 'border-[#0f172a]/20 hover:border-[#0f172a]/40 hover:bg-[#0f172a]/5 text-[#0f172a]'
            }`}
          >
            {t.lang_code}
          </button>
          ...
          <button className={`md:hidden p-2 ${isDark ? 'text-white' : 'text-[#0f172a]'}`} onClick={onMenuOpen}>
            <Menu size={24}/>
          </button>
        </div>
      </div>
